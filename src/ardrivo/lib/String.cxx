...
[[nodiscard]] int String::indexOf(const String& str) const noexcept {
    return static_cast<int>(m_u.find(str.m_u));
}
...
[[nodiscard]] String String::substring(unsigned from, unsigned to) const {
    return m_u.substr(from, to - from);
}
...
[[nodiscard]] long String::toInt() const noexcept try {
    return std::stoi(m_u);
} catch (const std::exception&) {
    return 0;
}

[[nodiscard]] double String::toDouble() const noexcept try {
    return std::stod(m_u);
} catch (const std::exception&) {
    return 0;
}

[[nodiscard]] float String::toFloat() const noexcept try {
    return std::stof(m_u);
} catch (const std::exception&) {
    return 0;
}
...
void String::toLowerCase() noexcept {
    std::transform(m_u.begin(), m_u.end(), m_u.begin(),
